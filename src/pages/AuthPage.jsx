...
import { Coffee, ChevronLeft, LogIn, Lock, Mail, AlertCircle, UserPlus, User } from 'lucide-react';
import { useAuthStore } from '../store/authStore';

export default function AuthPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const redirect = searchParams.get('redirect') || '/floor';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md bg-white/70 backdrop-blur-[80px] rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-slate-200/60 p-8 flex flex-col items-center overflow-hidden">
        
        <button 
          onClick={() => navigate(-1)}
          className="absolute top-6 left-6 flex items-center gap-2 text-slate-400 hover:text-indigo-600 font-semibold text-sm transition z-20"
        >
          <ChevronLeft className="w-4 h-4" /> Back
        </button>

        {/* Toggle Switch */}
        <div className="absolute top-6 right-6 flex bg-slate-100 rounded-full p-1 z-20">
          <button
            onClick={() => setIsLogin(true)}
            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${isLogin ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
          >
            Sign In
          </button>
          <button
        ...
        <div className="bg-indigo-600 text-white p-4 rounded-2xl shadow-lg mb-6 mt-8">
        ...
        <p className="text-slate-500 text-sm font-medium mb-8 text-center px-4">
          {isLogin ? 'Enter your credentials to access the terminal.' : 'Create a new customer account to order.'}
        </p>
