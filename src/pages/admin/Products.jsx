    ...
    <div className="p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Configure active food and beverages categories for the POS terminals.</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {categories.map((category) => (
            <button 
              ...
              className="px-5 py-4 bg-white border border-slate-200 hover:border-amber-500 hover:bg-amber-50/10 rounded-2xl font-semibold text-slate-800 hover:text-amber-700 transition-all text-center shadow-sm cursor-pointer"
            >
              {category.name}
            </button>
          ))}
